...
alias winHome="cd /mnt/c/Users/collinso"
function fgtab {
...
function make_keys {
if [[ $# -eq 0 ]]; then
	echo "Usage: \$key_type \$USER"
	sleep 10;
fi
key_type=$1	
local USER=$2
ssh-keygen -t rsa -b 2048 -C "$USER-$key_type-`date +%Y-%m-%d`" -f ./$USER-$key_type-`date +%Y-%m-%d`
}
